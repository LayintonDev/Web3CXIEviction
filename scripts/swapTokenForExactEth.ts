...
async function main() {
    ...
    const [amountIn] = await ROUTER.getAmountsIn(amountOut, [USDC, WETH]);

    let amountInMax = ethers.parseUnits(amountIn.toString(), 18);;
    ...
    const trx = await ROUTER.swapTokensForExactETH(
        amountOut,
        amountInMax,
        [USDC, WETH],
